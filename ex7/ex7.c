 ...
 *  r/R    Reverse direction on magenta face
 *  a      Toggle axes
...
int mode = 2; //  Depth mode
int rev = 0;  //  Reverse bottom of cube
int n = 1;    //  Number of cubes
const char *text[] = {"Face Culling", "Face Culling with Z-sort", "Z-buffer",
                      "Z-buffer + Face Culling"};
...
typedef struct {
  ...
  double Zp;         //  Projected Z value for Z-sort
} cube_t;
cube_t Cube[] = {
    {1, 0, 0, 0, 0.3, 0.3, 0.3, 0, 0},  {2, 0, 1, 0, 0.4, 0.4, 0.2, 45, 0},
    {3, 1, 0, 0, 0.2, 0.2, 0.2, 45, 0}, {4, -1, 0, 0, 0.2, 0.2, 0.2, 45, 0},
    {5, 0, 0, 1, 0.2, 0.2, 0.2, 45, 0}, {6, 0, 0, -1, 0.2, 0.2, 0.2, 45, 0},
};
...
void display() {
  ...
  //  Multiple cubes - fixed order
    ...
    for (int k = 0; k < n; k++)
      cube(Cube[k].x, Cube[k].y, Cube[k].z, Cube[k].dx, Cube[k].dy, Cube[k].dz,
           Cube[k].th);
  }
  //  Multiple cubes - Z-sorted
    ...
    //  Draw cubes
    for (int k = 0; k < n; k++)
      cube(cubep[k]->x, cubep[k]->y, cubep[k]->z, cubep[k]->dx, cubep[k]->dy,
           cubep[k]->dz, cubep[k]->th);
    //  Print sequence
    glColor3f(1, 1, 1);
    glWindowPos2i(5, 25);
    Print("Z order");
...
int main(int argc, char *argv[]) {
  ...
  if (depth_bits == 0)
    fprintf(stderr, "Error: Could not create a window with a depth buffer.\n");
